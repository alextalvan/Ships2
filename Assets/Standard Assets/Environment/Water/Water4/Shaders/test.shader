...
Properties {
	...
	_Shininess ("Shininess", Range (2.0, 10000.0)) = 200.0
	...
	//_CustomPerlin("Custom Perlin Value", Float) = 1.0
}
...
CGINCLUDE
	...
	v2f vert(appdata_full v)
	{
		...
		float3 worldSpaceVertex = mul(_Object2World,(v.vertex)).xyz;
		...
		//float2 tileableUv = mul(_Object2World,(v.vertex)).xz;	
		float vertMovement = 0.0;
		
		vertMovement+= sin(_CustomTime + (worldSpaceVertex.x) * _WaveScale.x ) * _WaveScale.y;
		vertMovement+= sin(_CustomTime + (worldSpaceVertex.z) * _WaveScale.z ) * _WaveScale.y;
		
		v.vertex.y += vertMovement;
		//test
		//float2 loc = worldSpaceVertex.xz;
		...
		o.bumpCoords.xyzw = (worldSpaceVertex.xzxz + _Time.xxxx * _BumpDirection.xyzw) * _BumpTiling.xyzw;
