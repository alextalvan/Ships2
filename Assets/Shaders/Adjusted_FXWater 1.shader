Shader "Custom/Adjsuted_FXWater" {
Properties {
	_ReflectionTex ("Internal reflection", 2D) = "white" {}
	
	_MainTex ("Fallback texture", 2D) = "black" {}
	_ShoreTex ("Shore & Foam texture ", 2D) = "black" {}
	_ShoreTex2 ("Shore & Foam texture2 ", 2D) = "black" {}
	_BumpMap ("Normals ", 2D) = "bump" {}
	
	_DistortParams ("Distortions (Bump waves, Reflection, Fresnel power, Fresnel bias)", Vector) = (0.58 ,1.12, 3.92, -0.31)
	_InvFadeParemeter ("Auto blend parameter (Edge, Shore, Distance scale)", Vector) = (0.3 ,0.39, 1.16, 3.22)
	
	_AnimationTiling ("Animation Tiling (Displacement)", Vector) = (0.04582553 , 0.0, 0.04571447, 0.02712969)
	_AnimationDirection ("Animation Direction (displacement)", Vector) = (2.0 ,4.0, 2.0, -1.0)

	_BumpTiling ("Bump Tiling", Vector) = (0.01 ,2.17, 0.08, 0.06)
	_BumpDirection ("Bump Direction & Speed", Vector) = (40, 54.8, 40.7, -40)
	
	_FresnelScale ("FresnelScale", Range (0.15, 4.0)) = 1.32

	_BaseColor ("Base color", COLOR)  = ( .007, .050, .039, 0.733)
	_ReflectionColor ("Reflection color", COLOR)  = ( 1.0, 1.0, 1.0, 0.352)
	_SpecularColor ("Specular color", COLOR)  = ( .047, .552, .490, 1)
	
	_WorldLightDir ("Specular light direction", Vector) = (-0.03037514, -0.2013908, -0.97904, 0.0)
	_Shininess ("Shininess", Range (2.0, 500.0)) = 22.0
	
	_Foam ("Foam (intensity, cutoff)", Vector) = (3.09, 0.39, -0.26, 0.98)
	
	_GerstnerIntensity("Per vertex displacement", Float) = 1.39
	_GAmplitude ("Wave Amplitude", Vector) = (0.14 ,-0.09, 0.175, 0.225)
	_GFrequency ("Wave Frequency", Vector) = (0.5, 0.56, 0.59, 0.6)
	_GSteepness ("Wave Steepness", Vector) = (7.33, 5.51, 6.0, 2.0)
	_GSpeed ("Wave Speed", Vector) = (-3, 2, 1, 3)
	_GDirectionAB ("Wave Direction", Vector) = (0.4691436 ,0.74, 0.64, 0.1)
	_GDirectionCD ("Wave Direction", Vector) = (0.7033888 ,-0.6799999, 0.7175735, -0.2)
...
CGINCLUDE
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		half4 foam2 = Foam(_ShoreTex2, i.bumpCoords  * 2.0);
		
		half4 foamFinal = lerp(foam,foam2,0.5);
		
		baseColor.rgb += foamFinal.rgb * _Foam.x * (edgeBlendFactors.y + saturate(i.viewInterpolator.w - _Foam.y));
